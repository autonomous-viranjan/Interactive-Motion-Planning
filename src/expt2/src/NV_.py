...
Use:
- `jstest-gtk` to get Wheel axis and button numbers
- `pyLinuxWheel` to set wheel resistance and other properties

    Drive using  : Logitech G920
...
class World(object):
    ...
    def restart(self):
        ...
        # Spawn the human driven car.
        blueprint = self.world.get_blueprint_library().filter('coupe_2020')[0]
...
class WheelControl(object):
    ...
    def __init__(self, world, start_in_autopilot):
        ...
        self.g920_sub = rospy.Subscriber("/g920_topic", Pose, self.g920_callback)

        # Control dynamics init
        self.a = 0
        self.l = 0
        self.tau_a = 0.1
        self.tau_s = 0.3
    ...
    def g920_callback(self, g920_msg):
        self.u_steer = g920_msg.position.x
        self.u_throttle = g920_msg.position.y
        self.brake = g920_msg.position.z
    ...
    # Wheel input based control
    def _parse_vehicle_control(self):
        # Get input from Logitech G920 Wheel
        # u_steer = self._wheel.get_axis(0)
        # u_throttle = -self._wheel.get_axis(1)
        # brake = abs(1 - self._wheel.get_axis(2))
        u_steer = self.u_steer
        u_throttle = self.u_throttle
        brake = self.brake

        # Controller Dynamics (simulator delta_seconds = 0.05)
        dt_sim = 0.05
        self.l = self.l + ((-1/self.tau_s)*self.l + (1/self.tau_s)*(u_steer + (u_steer - self.u_steer_prev)/dt_sim))*dt_sim
        self.a = self.a + ((-1/self.tau_a)*self.a + (1/self.tau_a)*u_throttle)*dt_sim

        self._control.steer = self.l
        self._control.throttle = max(self.a, 0)
        if brake > 0.1:
            self._control.brake = brake
        elif brake < 0.1 and self.a < 0.1:
...
def nv_publisher(position, velocity, acceleration):
    ...
    # looped in game_loop()
    vehicle_pose = Pose()
    vehicle_pose.position.x = position.x
    vehicle_pose.position.y = position.y
