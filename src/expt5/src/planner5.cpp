...
void PlannerNode::run(Imputer &Cost, Mpc &Opt, std::ofstream &logFile, std::ofstream &horizonFile)
{
    ...
    while (ros::ok()) {
        ...
        std::vector<double> plan = Opt.sol(X0, X0_NV, s_obs_, alpha_v, alpha_a, horizonFile);
...
void PlannerNode::dataLogger(std::ofstream &logFile, std::vector<double> &plan) {
    ...
    if (logFile.is_open()) {
        logFile << s_ << " " << v_ << " " << a_ << " " << l_ << " " << rl_ << " " 
        << "11111" << " " << plan[0] << " " << plan[1] << " " << plan[2] << " " << plan[3] << " " << plan[4] << " " << plan[5] << " " << plan[6] << " "
        << "11111" << " " << s_NV_ << " " << v_NV_ << " " << a_NV_ << " " << l_NV_ << " " << "11111" << " " << s_obs_ << " " 
        << "11111" << " " << alpha_v << " " << alpha_a << " "
        << "11111" << " " << plan[7] << " " << plan[8] << " " << plan[9] << std::endl;
    }
...
int main(int argc, char** argv)
{
    ...
    std::ofstream logFile ("src/expt5/data/log.txt"); // declare log file
    std::ofstream horizonFile ("src/expt5/data/horizon.txt");

    PlannerNode planner_node(&nh);

    planner_node.run(Cost, Opt, logFile, horizonFile);
